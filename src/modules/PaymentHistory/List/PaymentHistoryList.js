...
import PaymentHistoryListTable from "./MainTable/PaymentHistoryListTable";
import PaymentHistoryDialog from "../Dialog/PaymentHistoryDialog";
...
export class PaymentHistoryList extends ListBaseComponent {
  constructor(props) {
    super(props);
  ...
  onSearch = (currentPage = 1) => {
    ...
    PaymentHistoryApi.list(filter).then(res => {
  ...
  _OpenAddDialog = (isEdit = false, id = 0) => {
    let dialog = { ...this.state.PaymentHistoryDialog };
    dialog.show = true;
    dialog.isEdit = isEdit;
    dialog.id = id;
    this.setState({ PaymentHistoryDialog: dialog });
  };
  ...
  onDeleteItem = id => {
    PaymentHistoryApi.delete(id).then(res => {
      if (res && res.status === 200) {
  ...
  render() {
    return (
      <>        
		<PageName text='PaymentHistory' />
        <div className="page-content">
          <div className="search-div">
            <Row gutter={16}>
              <Col span={6} sm={12} md={6} xs={24}>
                <Input
                  placeholder="PaymentHistory name"
        ...
        {this.state.PaymentHistoryDialog.show ? (
          <PaymentHistoryDialog
            onCloseDialog={this._CloseAddDialog}
            {...this.state.PaymentHistoryDialog}
          />
        ) : null}
      </>
    );
...
export default withLoading(PaymentHistoryList);
