...
import RolesListTable from "./MainTable/RolesListTable";
...
export class RolesList extends ListBaseComponent {
  constructor(props) {
    super(props);
  ...
  _OpenAddDialog = (isEdit = false, id = 0) => {
    let dialog = { ...this.state.RolesDialog };
    dialog.show = true;
    dialog.isEdit = isEdit;
    dialog.id = id;
    this.setState({ RolesDialog: dialog });
  };
  ...
  render() {
    return (
      <>        
        ...
        {this.state.RolesDialog.show ? (
          <RolesDialog
            onCloseDialog={this._CloseAddDialog}
            {...this.state.RolesDialog}
          />
        ) : null}
      </>
    );
...
export default withLoading(RolesList);
