...
import TagCommentFilter from '../../common/CommonComponents/FormItemComponent/TagCommentFilter';
import AgreeCheckbox from '../../common/CommonComponents/FormItemComponent/AgreeCheckbox';


export default function FacebookVipComment() {
  const typeCode = TypeCode.facebookVip;
  const targetCode = "vip-comment-order";

  const [formData, setFormData] = useState({});
  const [code, setCode] = useState('');
  const [minPrice, setMinPrice] = useState(100);
  const [agreeRule, setAgreeRule] = useState(false);
  const [seedingType] = useState('vip_seeding_comment');
  ...
  function getCodeAndPrice() {
    var obj = getCategoryCodePrice(categories, seedingType, targetCode, false);
    if (obj) {
      setCode(obj.code);
  ...
  return (
    <>
      <PageName text="Buff Comment Facebook Vip" />
        ...
        <Row gutter={24} style={{ marginBottom: 24 }}>
          <Col lg={14}>
            <Form {...formItemLayout} className="login-form">
              ...
              <Form.Item
                label="Số lượng comment ngẫu nhiên"
              ...
              <Form.Item label="Bộ lọc từ khóa">
                <TagCommentFilter
                  value={formData.commentKeys}
                  onChange={value => onChangeFormItem(value, 'commentKeys')}
                />
                <span className="note">
                  Chỉ các bài viết có từ khóa mới tiến hành bình luận
                </span>
              </Form.Item>
              <Form.Item label="Nội dung Comment">
                <TextArea
...
const rule = (
  <>
    ...
    <br />
    <span className="bold-red">
      - Nghiêm cấm bình luận những nội có cử chỉ, lời nói thô bạo, khiêu khích, trêu ghẹo, xúc phạm nhân phẩm, danh dự của Cá nhân hoặc Tổ chức.<br />
      - Nếu cố tình buff bạn sẽ bị trừ hết tiền và band khỏi hệ thống vĩnh viễn, và phải chịu hoàn toàn trách nhiệm trước pháp luật.
    </span>
  </>
)