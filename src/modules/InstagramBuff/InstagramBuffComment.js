...
export default function InstagramBuffComment() {
  const typeCode = TypeCode.instagramBuff;
  const targetCode = "order-instagram-comment";
  const seedingTypes = [{ name: 'Tăng Comment Instagram', value: 'instagram_buff_comment' }];
  ...
  useEffect(() => {
    ...
    newFormData.numberBuff = newFormData.comments ? newFormData.comments.split("\n").filter(c => c).length : 0;
  ...
  function onChangeFormItem(value, field) {
    ...
    if (field === 'comments' && newFormData[field] !== null && newFormData[field] !== undefined) {
      newFormData.numberBuff = newFormData.comments.split("\n").filter(c => c).length;
    }
    setFormData(newFormData);
  }
  ...
  function onSubmit() {
    ...
    submitData.totalCoin = calTotal(formData);
    ...
    <>
      ...
      <div className="page-content facebook-buff-div">
        ...
        <Row style={{ marginBottom: 10 }}>
          <Col span={6}></Col>
          <Col span={16}>
            <Checkbox onChange={e => setAgreeRule(e.target.checked)}>Đồng ý với các quy định</Checkbox>
          </Col>
        </Row>

        <Row style={{ display: 'flex', justifyContent: 'center', marginTop: 10 }}>
          <ButtonSubmit disabled={!agreeRule} total={calTotal(formData, minPrice)} onSubmit={onSubmit} />
        </Row>
      </div>
    </>
  )
