...
export class RoleSiteDialog extends Component {
  constructor(props) {
    super(props);
  ...
  handleOk = () => {
    this.props.form.validateFields((err, values) => { 
      ...
      if (this.props.onCloseDialog) {
        RoleSiteApi.update(submitData).then(res => {
          if (res) {
  ...
  componentDidMount() {
    ...
    if (this.props.id) {
      RoleSiteApi.getById(this.props.id).then(res => {
  ...
  render() {
    ...
    return (
      <>
        <Modal
          title={this.props.isEdit ? "Update RoleSite" : "Add RoleSite"}
          visible={true}
          onOk={this.handleOk}
...
export default Form.create({ name: "form" })(RoleSiteDialog);
